...
if sys.argv[1] == "txone":
    ...
    # Dedicated ICMP test for bonus points
    import struct
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    # SERVERPORT is calculated based on UID
    sock.bind(('127.0.0.1', SERVERPORT))
    print("icmp: listening for a packet to reject...")

    try:
        # Wait for the trigger packet from xv6 nettest
        data, addr = sock.recvfrom(1024)
        print(f"icmp: received packet from {addr}, sending ICMP Unreachable...")
        # By closing the socket, the host OS (Linux/Mac) will
        # automatically send an ICMP Destination Unreachable
        # back to xv6 when it tries to receive.
        sock.close()
    except Exception as e:
        print(f"icmp: error occurred: {e}")
else:
    usage()
