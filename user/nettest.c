...
icmp_test()
{
  printf("icmp_test: starting\n");
  
  uint32 dst = MAKE_IP_ADDR(10, 0, 2, 2); // QEMU Host Gateway
  int local_port = 2025;
  int closed_port = 9999; // A port that is not listening
  
  bind(local_port);

  // Send a packet to trigger the ICMP response
  if (send(local_port, dst, closed_port, "trigger", 7) < 0) {
    printf("icmp_test: send failed\n");
    return 0;
  }

  printf("icmp_test: waiting for ICMP error...\n");
  
  char ibuf[128];
  uint32 src;
  uint16 sport;
  
  // recv() should return -1 because icmp_rx will set s->err = 1
  int cc = recv(local_port, &src, &sport, ibuf, sizeof(ibuf));

  if (cc < 0) {
    printf("icmp_test: OK (received expected error)\n");
    return 1;
  } else {
    printf("icmp_test: FAILED (recv returned %d instead of -1)\n", cc);
    return 0;
  }
}
