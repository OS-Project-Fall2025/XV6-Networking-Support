...
e1000_recv(void)
{
  // check the next idx for tail
  int idx = (regs[E1000_RDT] + 1) % RX_RING_SIZE;
  // loop on ring to empty the slots for burst
  while(rx_ring[idx].status & E1000_RXD_STAT_DD) {
    //pass the packet buffer to os
    char *buf = (char *)rx_ring[idx].addr;
    int len = rx_ring[idx].length;
    net_rx(buf, len);
    // alloc new buffer to empty ring
    char *new_buf = kalloc();
    if(new_buf == 0){
      panic("e1000_recv: kalloc failed");
    }
    rx_ring[idx].addr = (uint64)new_buf;
    //pass the ring to hardware
    rx_ring[idx].status = 0;
    //giving the idx to hardware
    regs[E1000_RDT] = idx;
